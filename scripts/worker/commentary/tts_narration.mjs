...
//   ELEVENLABS_VOICE_ID (overrides track.yaml tts.voice_id; default 'Brian')
...
// Per-task override (COMMENTARY_VOICE) wins over track-level default.
// Env precedence: COMMENTARY_VOICE > ELEVENLABS_VOICE_ID > track.yaml tts.voice_id > 'Brian'
...
async function createKieTask() {
  ...
  const input = {
    dialogue: [{ voice: voiceId, text: fullText }],
    stability,
  };
...
async function pollRecordInfo(taskId) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      // state in: waiting | queuing | generating | running — keep polling
      console.log(`[tts] state=${state || '?'} taskId=${taskId}`);
    } catch (e) {
      // network or parse errors during polling: log and retry unless caller threw
