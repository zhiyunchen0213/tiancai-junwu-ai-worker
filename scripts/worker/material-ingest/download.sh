...
download_local() {
  ...
  # Cookie strategy:
  # - douyin / tiktok: use Safari browser cookie store (Safari needs to have visited
  #   the site at least once to seed visitor cookies; no login required for douyin
  #   per yt-dlp error msg "Fresh cookies (not necessarily logged in) are needed")
...
download_remote() {
  ...
  local cookies_flag=""
  if [[ "$SOURCE_URL" =~ douyin\.com|tiktok\.com ]]; then
    cookies_flag="--cookies-from-browser chrome"
